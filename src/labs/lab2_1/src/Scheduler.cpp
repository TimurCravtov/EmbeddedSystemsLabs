...
namespace Scheduler {
    ...
    void addTask(void (*taskFunction)(), uint16_t recurrenceDelay, uint16_t offset) {
        ...
        if (temp != nullptr) {
            ...
            tasks[taskCount].recurrenceDelay = recurrenceDelay;
            tasks[taskCount].offset = offset;
            tasks[taskCount].recurrenceControl = offset;
            taskCount++;
        } else {
    ...
    // this function should add the tasks to the scheduler using addTask function
    extern void setup();
